...
通过决策树我们可以管理回归和分类问题。决策树是随机森林的基础(合并多个决策树）。当我们使用决策树时，我们不需要标准化数据。

## 决策树的训练和可视化

![image](https://user-images.githubusercontent.com/121781739/227159598-4141583c-908b-48df-adf5-4fcc45a92d67.png)

基尼系数衡量杂质，samples显示我们在每个节点有几个观察对象。value显示每个类型有几个观察对象。class显示预测的分类。
...
硬投票（hard voting）：对于每个输入样本，每个分类器都会投出一个标签，最终的分类结果是通过简单多数投票决定的。

软投票（soft voting）：对于每个输入样本，每个分类器都会投出一个概率分布，最终的分类结果是通过加权平均投票决定的。

Voting classifiers 的优点在于可以将多个不同的分类器的优点结合起来，从而获得更好的分类性能。例如，可以将一个支持向量机、一个决策树和一个随机森林组合起来，从而得到一个更为强大的分类器。

## Bagging and Pasting

Bagging（Bootstrap aggregating）和 Pasting 是两种常见的集成学习方法，用于将多个基分类器组合成一个更强大的分类器。
