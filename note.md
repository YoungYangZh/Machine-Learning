...
数据标准化：对原始数据进行标准化处理，使得每个特征的均值为 0，方差为 1。

计算协方差矩阵：对标准化后的数据计算协方差矩阵，用来描述各个特征之间的相关性。

计算特征值和特征向量：对协方差矩阵进行特征值分解，得到协方差矩阵的特征值和对应的特征向量。

选择主成分：根据特征值的大小，选择前 k 个特征向量作为主成分，其中 k 是目标降维后的维度。

生成新的特征空间：将原始数据映射到选定的前 k 个特征向量构成的新的特征空间中，得到降维后的数据。
...
是一种非线性降维技术，它通过保持数据之间的局部线性关系来进行降维。LLE 的基本思想是将每个数据点表示为其 k 个最近邻数据点的线性组合，然后在低维空间中保持这些线性关系。

LLE 的算法步骤如下：

找到每个数据点的 k 个最近邻点，并计算每个数据点与其最近邻点之间的权重。

构建权重矩阵 W，其中每个元素 w(i,j) 表示第 i 个数据点与第 j 个数据点之间的权重。

构建重建权重矩阵 W’，其中每个元素 w’(i,j) 表示用第 i 个数据点的 k 个最近邻点的线性组合来重建第 j 个数据点时，第 j 个数据点对第 i 个数据点的权重。

计算低维表示，将数据点从高维空间映射到低维空间，使得重建权重矩阵 W’ 的误差最小。
